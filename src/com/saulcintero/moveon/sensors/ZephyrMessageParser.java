...
public class ZephyrMessageParser implements MessageParser {
	...
	public ZephyrMessageParser(Context context) {
		this.mContext = context;
	}

	@Override
	public Sensor.SensorDataSet parseBuffer(byte[] buffer) {
		Sensor.SensorDataSet.Builder sds = Sensor.SensorDataSet.newBuilder().setCreationTime(
				System.currentTimeMillis());

		Sensor.SensorData.Builder heartrate = Sensor.SensorData.newBuilder().setValue(buffer[12] & 0xFF)
				.setState(Sensor.SensorState.SENDING);
		sds.setHeartRate(heartrate);

		Sensor.SensorData.Builder batteryLevel = Sensor.SensorData.newBuilder().setValue(buffer[11])
				.setState(Sensor.SensorState.SENDING);
	...
	private void setCadence(Sensor.SensorDataSet.Builder sds, byte[] buffer) {
		...
		// Firmware values range from field 3 to 10 (inclusive) of the byte
		// buffer.
		byte[] hardwareFirmwareId = copyByteArray(buffer, 3, 11);
		...
		if (Arrays.equals(hardwareFirmwareId, CADENCE_BUG_FW_ID)) {
			...
			if (strideReadings.getCadence() != StrideReadings.CADENCE_NOT_AVAILABLE) {
				finalCadence = strideReadings.getCadence();
				cadence.setValue(strideReadings.getCadence()).setState(Sensor.SensorState.SENDING);
			}
		} else {
			finalCadence = (SensorUtils.unsignedShortToIntLittleEndian(buffer, 56) / 16);
			cadence.setValue(SensorUtils.unsignedShortToIntLittleEndian(buffer, 56) / 16).setState(
					Sensor.SensorState.SENDING);
		}

		Intent i = new Intent("android.intent.action.BEAT_COUNTER");
		i.putExtra("beats", String.valueOf(finalCadence));
		mContext.sendBroadcast(i);

		sds.setCadence(cadence);
	}

	private byte[] copyByteArray(byte[] input, int start, int end) {
	...
	public boolean isValid(byte[] buffer) {
		...
		return buffer.length > ZEPHYR_HXM_BYTE_ETX && buffer[ZEPHYR_HXM_BYTE_STX] == 0x02
