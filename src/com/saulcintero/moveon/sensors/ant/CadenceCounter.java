...
public class CadenceCounter {
	...
	public int getEventsPerMinute(int count, int eventTime) {
		...
		if (lastCount < 0) {
			/*
			 * eventTime for the initial count value is probably out of date, so
		...
		if (countChange != 0) {
			...
			/*
			 * Too much time has passed since the last count change. A smaller
			 * value than eventsPerMinute must be returned. The value is
			 * calculated from the history.
			 */
	...
	private int getValueFromHistory(long now) {
		...
		int systemTimeChange = (int) (now - lastCadenceData.systemTime + (eventTimeChange * MILLIS_PER_MINUTE)
				/ EVENT_TIME_PER_MINUTE);
		...
		/*
		 * Do not return a larger number than eventsPerMinute because this
		 * function is only called when more time has passed after the last
		 * count change than the interval from eventsPerMinute.
		 */
