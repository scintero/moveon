...
import com.saulcintero.moveon.sensors.Sensor;
...
public class AntSensorManager extends SensorManager {
	...
	private AntSensorValue antSensorValue;
	...
	public AntSensorManager(Context context) {
		...
		antSensorValue = new AntSensorValue(context);
	...
	private void tryClaimAnt() {
		try {
			antInterface.requestForceClaimInterface(context.getString(R.string.app_name));
		} catch (AntInterfaceException e) {
			handleAntError();
		}
	}
	...
	private void openChannel(byte channel) {
		short deviceNumber = (short) Constants.getInt(context, channelConfig[channel].getDeviceIdKey(),
				WILDCARD);
		...
		public void onReceive(Context c, Intent intent) {
			if (intent.getAction().equals(AntInterfaceIntent.ANT_RX_MESSAGE_ACTION)) {
				...
				case AntMesg.MESG_CHANNEL_ID_ID:
					...
					channelConfig[channel].setDeviceNumber(deviceNumber);
					Constants.setInt(context, channelConfig[channel].getDeviceIdKey(), deviceNumber);
		...
		private void handleResponseEventMessage(byte[] message) {
			...
			if (channelConfig[channel].isInitializing()) {
				if (message[AntMesg.MESG_DATA_OFFSET + 2] != 0) {
					...
					case AntMesg.MESG_ASSIGN_CHANNEL_ID:
						try {
							antInterface.ANTSetChannelId(channel, channelConfig[channel].getDeviceNumber(),
									channelConfig[channel].getDeviceType(),
									ChannelConfiguration.TRANSMISSION_TYPE);
						} catch (AntInterfaceException e) {
							handleAntError();
						}
						break;
					case AntMesg.MESG_CHANNEL_ID_ID:
						try {
							antInterface.ANTSetChannelPeriod(channel,
									channelConfig[channel].getMessagPeriod());
						} catch (AntInterfaceException e) {
							handleAntError();
						}
						break;
					...
					case AntMesg.MESG_SET_LP_SEARCH_TIMEOUT_ID:
						if (channelConfig[channel].getDeviceNumber() == WILDCARD) {
							try {
								...
								// card search
								antInterface.ANTSetProximitySearch(channel,
										ChannelConfiguration.PROXIMITY_SEARCH);
							} catch (AntInterfaceException e) {
								handleAntError();
							}
						} else {
	...
	private void setSensorDataSet() {
		...
		if (heartRate != -1) {
			builder.setHeartRate(Sensor.SensorData.newBuilder().setValue(heartRate)
					.setState(Sensor.SensorState.SENDING));
		}
		int cadence = antSensorValue.getCadence();
		if (cadence != -1) {
			builder.setCadence(Sensor.SensorData.newBuilder().setValue(cadence)
					.setState(Sensor.SensorState.SENDING));
		}
	...
	private boolean isAirPlaneMode() {
		if (!Settings.System.getString(context.getContentResolver(), Settings.System.AIRPLANE_MODE_RADIOS)
				.contains(RADIO_ANT)) {
			return false;
		}
