...
public class BluetoothConnectionManager {
	 ...
	 * @param handler
	 *            a hander for sending messages back to the UI activity
	 * @param messageParser
	 *            a message parser
	 */
	...
	private BluetoothSocket getBluetoothSocket(BluetoothDevice bluetoothDevice) throws IOException {
		try {
			return bluetoothDevice
					.createInsecureRfcommSocketToServiceRecord(BluetoothConnectionManager.MY_TRACKS_UUID);
		} catch (IOException e) {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					// Send a copy of the obtained bytes to the handler to avoid
					// memory
