...
public class SensorUtils {
	 ...
	 *
	 * @param buffer
	 *            the buffer to extract the short from
	 * @param index
	 *            the first byte to be interpreted as part of the short
	 ...
	 *
	 * @param buffer
	 *            the byte array of data (payload)
	 * @param start
	 *            the position in the byte array where the payload begins
	 * @param length
	 *            the length
	 * @return CRC8 value
	 */
	 ...
	 *            int of crc value
	 * @param add
	 *            the next byte to add to the CRC8 calculation
	 */
	private static byte crc8PushByte(byte crc, byte add) {
		...
		for (int i = 0; i < 8; i++) {
			if ((crc & 0x1) != 0x0) {
				// Using a 0xFF bit assures that 0-bits are introduced during
				// the shift operation.
				// Otherwise, implicit casts to signed int could shift in 1-bits
				// if the signed bit is 1.
